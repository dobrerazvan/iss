...
!#SimDataAdd
    integer, parameter :: nTomoBin_max = 20
    integer, parameter :: cl_zbins_max = 40
    integer, parameter :: cl_Mbins_max = 100
!#SimDataAdd
...
!#SimDataAdd
    logical   :: DoCS, DoGal, DoCSXGal, DoClusters, DoCMB
    logical   :: OutputSimDataFiles
    logical   :: sim_random_cmb
    integer   :: sim_random_seed
    logical   :: DoShePowFoc = .false.
    logical   :: CSGalCosmoMC = .false.
    logical   :: doDvir = .false.
!#SimDataAdd
...
!#SimDataAdd
    integer   :: nTomoBin(1:2)
    real(dl)  :: zph_low(1:nTomoBin_max,1:2)
    real(dl)  :: zph_high(1:nTomoBin_max,1:2)
    integer   :: lmax_CS, lmax_gal, lmax_cmb, lmin_CS, lmin_gal, lmin_cmb
    real(dl)  :: fskycmb
    integer   :: sim_ncmbcls,nchan
    real(dl)  :: photo_error
    real(dl)  :: nz_z0_lss,nz_alpha_lss,nz_beta_lss,ngal_lss
    real(dl)  :: mean_int_ellip_lss,fskylss
    real(dl)  :: cs2_de
    integer   :: cl_zbins,cl_Mbins
    real(dl)  :: ngal_cl,fskycl,lnM_sigma,completeness,efficiency
    real(dl)  :: nz_z0_cl,nz_alpha_cl,nz_beta_cl,zmin_cl,zmax_cl
    real(dl)  :: c_nfw,theta_G,mean_int_ellip_cl,w0,wa,signal2noise
    real(dl)  :: cl_taui = 2.d-6
    real(dl)  :: cl_M = 1.d16
    real(dl)  :: cl_RedshiftBins(cl_zbins_max+1),cl_MassBins(cl_zbins_max,cl_Mbins_max+1)
    real(dl), dimension(:), allocatable :: Noise_Sigma_T,Noise_Sigma_P,fwhm_arcmin
!#SimDataAdd
...
!        integer :: nc,ll
!        real(dl) :: NoiseCl(2,lmin_cmb:lmax_cmb)
!        real(dl) :: sigma2(P%nchan),fwhm_rad(P%nchan)
!        real(dl) :: TT,EE,TE,BB,xlc
!        character(LEN=100) :: setoutname,cmboutname

!        cmboutnate = trim(datarep)//trim(setname)//"_cmb.dat"
...
!                                1.d0/((fwhm_rad(nc)*Sigma_T(nc))**2) &
!                                *exp(real(-ll*(ll+1),KIND(1.d0))*sigma2(nc))
!                        NoiseCl(2,ll) = NoiseCl(2,ll) + &
!                                1.d0/((fwhm_rad(nc)*Sigma_P(nc))**2) &
!                                *exp(real(-ll*(ll+1),KIND(1.d0))*sigma2(nc))
!                end do
!        end do

!        NoiseCl(1,:) = 1.d0/NoiceCl(1,:)
!        NoiseCl(2,:) = 1.d0/NoiceCl(2,:)
...
!                        BB = Cl_tensor(ll,1,CT_B)
!                else
!                        BB = 0.d0
!                end if

!                TT = TT*2.d0*pi/real(ll*(ll+1),KIND(1.d0))*P%t_cmb**2 + &
!                        NoiseCl(1,ll)
!                EE = EE*2.d0*pi/real(ll*(ll+1),KIND(1.d0))*P%t_cmb**2 + &
!                        NoiseCl(2,ll)
!                TE = TE*2.d0*pi/real(ll*(ll+1),KIND(1.d0))*P%t_cmb**2
!                if(P%WantTensors .and. ll .le. P%Max_l_tensor) then
!                        BB = BB*2.d0*pi/real(ll*(ll+1),KIND(1.d0))*P%t_cmb**2 + &
!                                NoiseCl(2,ll)
!                end if

!                write(34,'(1I5,13E17.8)') l, TT, TE, 0.d0, TE, EE, 0.d0, 0.d0, &
!                                0.d0, BB, NoiseCl(1,ll), NoiseCl(2,ll), P%fskycmb
!        end do
...
!        write (37,'(A,I5)') "sim_ncmbcls = ",num_cls
!        write (37,'(A,A)')  "CMBCl_theo = ",trim(cmbcl_infile)
!        if (num_cls == 5) write (37,'(A,A)')  "CMBLensCl_theo = ",trim(cmbcllens_infile)
!        write (37,'(A,I5)') "sim_lmax = ",cmb_lmax
!        write (37,'(A,I5)') "sim_lmin = ",cmb_lmin
!        write (37,'(A,A)')  "datafile = ",trim(cmboutname)
!        close(37)
