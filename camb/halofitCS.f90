    ...
    public Min_kh_nonlinear, NonLinear_GetNonLinRatiosCS, NonLinear_ReadParams
    ...
    subroutine NonLinear_GetNonLinRatiosCS(CAMB_Pk)
...
!SimDataReplace
            ...
            om_m = omega_m(a, omm0, CP%omegav, CP%w0, CP%wa)  
            om_v = omega_v(a, omm0, CP%omegav, CP%w0, CP%wa)
!!SimDataReplace
            acur = a
            xlogr1=-2.0
            xlogr2=3.5
...
101         continue
    ...
    end subroutine NonLinear_GetNonLinRatiosCS
...
!#SimDataReplace
        ...
        a=1.5222+2.8553*rn+2.3706*rn*rn+0.9903*rn*rn*rn+ &
            0.2250*rn*rn*rn*rn-0.6038*rncur+0.1749*om_v*(1.+CP%w0+wa_ppf*(1-acur))
!#SimDataReplace
        a=10**a
!#SimDataReplace
        ...
        b=10**(-0.5642+0.5864*rn+0.5716*rn*rn-1.5474*rncur+ &
            0.2279*om_v*(1.+CP%w0+wa_ppf*(1-acur)))
!!! atentie ca wa_ppf sa fie 0
!print*, 'In halofit_ppf wa_ppf =', wa_ppf
!#SimDataReplace
...
!    #Simdata : aproximativ aceeasi generalizare, dar pastreaza parametrizarea din SimData. De verificat
...
!    end function omega_m
     ...
     function omega_m(aa,om_m0,om_v0,w0val,waval)
     implicit none
     real(dl) omega_m,omega_t,om_m0,om_v0,aa,w0val,waval,v_fac
     v_fac= aa**(-1.0-3.0*(w0val+waval))*dexp(-3.0*(1-aa)*waval)
     omega_t=1.0+(om_m0+om_v0-1.0)/(1-om_m0-om_v0+om_v0*v_fac+om_m0/aa)
     omega_m=omega_t*om_m0/(om_m0+om_v0*aa*v_fac)
     end function omega_m
...
!    end function omega_v
      ...
      function omega_v(aa,om_m0,om_v0,w0val,waval)      
      implicit none
      real(dl) aa,omega_v,om_m0,om_v0,omega_t,w0val,waval,v_fac
      v_fac = aa**(-1.d0-3.d0*(w0val+waval))*exp(-3.d0*waval*(1.d0-aa))
      omega_t=1.0+(om_m0+om_v0-1.0)/(1-om_m0-om_v0+om_v0*v_fac+om_m0/aa)
      omega_v=omega_t*om_v0*v_fac/(om_v0*v_fac+om_m0/aa)
      end function omega_v
!#SimDataReplace
    ...
    end module NonLinearCS


    !workaround for f90 circular-module reference
    subroutine NonLinear_GetRatiosCS(CAMB_Pk)
    use TransferCS
    use NonLinearCS
    type(MatterPowerData) :: CAMB_Pk

    call NonLinear_GetNonLinRatiosCS(CAMB_Pk)

    end subroutine NonLinear_GetRatiosCS



    subroutine NonLinear_GetRatios_allCS(CAMB_Pk)
    use TransferCS
    use NonLinearCS
    type(MatterPowerData) :: CAMB_Pk
    ...
    end subroutine NonLinear_GetRatios_AllCS
