...
contains
  ...
  subroutine calc_Nldd (incls, inlcls, clnoise, nldd, jjmaxTmax, lmaxM)
    ...
    REAL,           INTENT(IN), DIMENSION (:,:) :: incls
    !! Cl non lentilles : 1=T, 2=E, 3=TE, 4=dd, 5=Td
    REAL,           INTENT(IN), DIMENSION (:,:) :: inlcls
    !! Cl lentilles : 1=T, 2=E, 3=TE, 4=B
    REAL,           INTENT(IN), DIMENSION (:,:) :: clnoise
    !! Bruit instru : 1=T, 2=P
    double precision,INTENT(OUT), DIMENSION (:) :: nldd
    !! la memoire pour nldd doit etre deja allouee avant l'appel a calc_nldd
    INTEGER,       INTENT(IN) :: jjmaxTmax 
    ...
    if (jjmaxT.gt.jjmaxTmax) then
       write(*,*)'Please increase parameter jjmaxTmax to ',jjmaxT
       stop
    end if
