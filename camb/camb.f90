...
!#SimData
    if(Params%DoShePowFoC) then
        if ((.not. Params%CSGalCosmoMC) .and. (Params%DoCS .or. Params%DoGal)) & 
        call ShePowFoCDriver(Params%DoCS,Params%DoGal,Params%DoCSXGal,Params,dumy)
        if ((.not. Params%CSGalCosmoMC) .and. Params%DoClusters) call ClusterFoCDriver(Params,dumy2)
    end if
!#SimData
